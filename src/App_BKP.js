...
function App() {
    ...
    const applyFilter = () => {
        ...
        // Combine the header rows with the updated filtered data rows
        const outputRows = filteredRows.length > 0 ? [...headerRows, ...[rows[3]], ...filteredRows] : [...headerRows, ...[rows[3]], ...rows.filter(e => Number.isInteger(e[0]))]
        const totalFamilies = filteredRows.length > 0 ? filteredRows.length : rows.filter(e => Number.isInteger(e[0])).length + 1;
        outputRows.push(['', 'TOTAL NO OF FAMILIES', totalFamilies])
        outputRows.push(['', 'TOTAL NO OF SOULS', rows[3][3] > 0 ? rows[3][3] : 'NIL'])
        outputRows.push(['', 'TOTAL RICE ISSUED', rows[3][4] > 0 ? rows[3][4] : 'NIL'])
        outputRows.push(['', 'TOTAL ATTA ISSUED', rows[3][5]> 0 ? rows[3][5] : 'NIL'])
        outputRows.push(['', 'TOTAL SUGAR ISSUED', rows[3][6] > 0 ? rows[3][6] : 'NIL'])
